...
// stage2/3/4 (stage2=double buffers+copy async), 128x128, mma4x2,
// warp2x4(32,64,16)
...
__global__ void __launch_bounds__(256)
    hgemm_wmma_m16n16k16_mma4x2_warp2x4_stages_kernel(half *A, half *B, half *C,
                                                      int M, int N, int K) {
...
template <const int WMMA_M = 16, const int WMMA_N = 16, const int WMMA_K = 16,
          ...
          const bool BLOCK_SWIZZLE = false>
__global__ void __launch_bounds__(256)
    hgemm_wmma_m16n16k16_mma4x2_warp2x4_stages_dsmem_kernel(half *A, half *B,
                                                            half *C, int M,
                                                            int N, int K) {
  // 256 threads(8 warps) per block.
  // const int bx = blockIdx.x;
...
__global__ void __launch_bounds__(512)
    hgemm_wmma_m16n16k16_mma4x4_warp4x4_stages_dsmem_kernel(half *A, half *B,
                                                            half *C, int M,
                                                            int N, int K) {
  ...
  // 先计算shared memory中的索引
  // tid和需要加载的smem s_a[BM][BK] 之间的索引关系 BM=256 BK=16 按行读取
  // A行主序
  // 对于s_a每行16个数据，每个线程读取8个，需要2个线程；总共256行，需要刚好256x2=512线程
  int load_smem_a_m = tid / 2;                // row 0~255
  int load_smem_a_k = (tid % 2 == 0) ? 0 : 8; // col 0, 8
  // tid和需要加载的smem s_b[BK][BN] 之间的索引关系 BK=16 BN=256 按行读取
  // B行主序
...
template <const int WMMA_M = 16, const int WMMA_N = 16, const int WMMA_K = 16,
          const int WMMA_TILE_M = 4, const int WMMA_TILE_N = 2,
          const int WARP_TILE_M = 4, const int WARP_TILE_N = 4,
          const int WARP_TILE_K = 1, const int A_PAD = 0, const int B_PAD = 0,
          const int K_STAGE = 2, const bool BLOCK_SWIZZLE = false>
__global__ void __launch_bounds__(256)
    hgemm_wmma_m16n16k16_mma4x2_warp4x4_stages_dsmem_kernel(half *A, half *B,
                                                            half *C, int M,
                                                            int N, int K) {
  // 256 threads(8 warps) per block.
  // const int bx = blockIdx.x;
  ...
  // 先计算shared memory中的索引
  // tid和需要加载的smem s_a[BM][BK] 之间的索引关系 BM=256 BK=32 按行读取
  // A行主序
...
#pragma unroll
  for (int k = 0; k < (K_STAGE - 1); ++k) { // 0, 1
    // k * WMMA_K, WMMA_K=16 -> (k << 4)
    int load_gmem_a_k =
        k * (WMMA_K * WARP_TILE_K) + load_smem_a_k; // global col of a
    int load_gmem_a_addr = load_gmem_a_m * K + load_gmem_a_k;
    int load_gmem_b_k =
        k * (WMMA_K * WARP_TILE_K) + load_smem_b_k; // global row of b
    int load_gmem_b_addr = load_gmem_b_k * N + load_gmem_b_n;
...
#pragma unroll
      for (int j = 0; j < WARP_TILE_N; ++j) {
        ...
        int warp_smem_b_n = warp_n * (WMMA_N * WARP_TILE_N) + j * WMMA_N;
        half *load_smem_b_frag_ptr =
            (s_b + smem_sel * s_b_stage_offset + warp_smem_k * (BN + B_PAD) +
             warp_smem_b_n);
        wmma::load_matrix_sync(B_frag[j], load_smem_b_frag_ptr, BN + B_PAD);
      }
...
#pragma unroll
        for (int i = 0; i < WARP_TILE_M; ++i) {
          ...
          int warp_smem_a_m = warp_m * (WMMA_M * WARP_TILE_M) + i * WMMA_M;
          half *load_smem_a_frag_ptr =
              (s_a + stage_sel * s_a_stage_offset +
               warp_smem_a_m * (BK + A_PAD) + warp_smem_k);
          wmma::load_matrix_sync(A_frag[i], load_smem_a_frag_ptr, BK + A_PAD);
        }
...
#pragma unroll
        for (int j = 0; j < WARP_TILE_N; ++j) {
          ...
          int warp_smem_b_n = warp_n * (WMMA_N * WARP_TILE_N) + j * WMMA_N;
          half *load_smem_b_frag_ptr =
              (s_b + stage_sel * s_b_stage_offset + warp_smem_k * (BN + B_PAD) +
               warp_smem_b_n);
          wmma::load_matrix_sync(B_frag[j], load_smem_b_frag_ptr, BN + B_PAD);
        }
