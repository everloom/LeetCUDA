...
template <const int BM = 128, const int BN = 128, const int BK = 16,
          const int TM = 8, const int TN = 8, const int OFFSET = 0>
__global__ void
hgemm_t_8x8_sliced_k16_f16x8_pack_dbuf_async_kernel(half *a, half *b, half *c,
                                                    int M, int N, int K) {
  int bx = blockIdx.x;
  int by = blockIdx.y;
  int tx = threadIdx.x;
...
template <const int BM = 128, const int BN = 128, const int BK = 32,
          const int TM = 8, const int TN = 8, const int OFFSET = 0>
__global__ void hgemm_t_8x8_sliced_k32_f16x8_pack_dbuf_kernel(half *a, half *b,
                                                              half *c, int M,
                                                              int N, int K) {
  int bx = blockIdx.x;
  int by = blockIdx.y;
  ...
  // tid和需要加载的smem s_b[BK][BN] 之间的索引关系 BK=32 BN=128 按行读取
  // B行主序
...
template <const int BM = 128, const int BN = 128, const int BK = 32,
          const int TM = 8, const int TN = 8, const int OFFSET = 0>
__global__ void
hgemm_t_8x8_sliced_k32_f16x8_pack_dbuf_async_kernel(half *a, half *b, half *c,
                                                    int M, int N, int K) {
  int bx = blockIdx.x;
  int by = blockIdx.y;
  int tx = threadIdx.x;
...
__global__ void hgemm_t_16x8_sliced_k32_f16x8_pack_dbuf_kernel(half *a, half *b,
                                                               half *c, int M,
                                                               int N, int K) {
  ...
  // 0. 先计算shared memory中的索引
  // tid和需要加载的smem s_a[BM][BK] 之间的索引关系 BM=128 BK=32 按行读取
  // A行主序
...
template <const int BM = 128, const int BN = 128, const int BK = 32,
          const int TM = 16, const int TN = 8, const int OFFSET = 0>
__global__ void
hgemm_t_16x8_sliced_k32_f16x8_pack_dbuf_async_kernel(half *a, half *b, half *c,
                                                     int M, int N, int K) {
  // block(BN/TN, BM/TM) -> (x=16, y=8)
  int bx = blockIdx.x;
  int by = blockIdx.y;
...
void hgemm_t_8x8_sliced_k16_f16x8_pack_dbuf(torch::Tensor a, torch::Tensor b,
  ...
  hgemm_t_8x8_sliced_k16_f16x8_pack_dbuf_kernel<BM, BN, BK, TM, TN, 8>
...
void hgemm_t_8x8_sliced_k16_f16x8_pack_dbuf_async(torch::Tensor a,
  ...
  hgemm_t_8x8_sliced_k16_f16x8_pack_dbuf_async_kernel<BM, BN, BK, TM, TN, 8>
      <<<grid, block>>>(reinterpret_cast<half *>(a.data_ptr()),
                        reinterpret_cast<half *>(b.data_ptr()),
                        reinterpret_cast<half *>(c.data_ptr()), M, N, K);
}
...
void hgemm_t_8x8_sliced_k32_f16x8_pack_dbuf(torch::Tensor a, torch::Tensor b,
  ...
  hgemm_t_8x8_sliced_k32_f16x8_pack_dbuf_kernel<BM, BN, BK, TM, TN, 8>
      <<<grid, block>>>(reinterpret_cast<half *>(a.data_ptr()),
                        reinterpret_cast<half *>(b.data_ptr()),
                        reinterpret_cast<half *>(c.data_ptr()), M, N, K);
}
...
void hgemm_t_8x8_sliced_k32_f16x8_pack_dbuf_async(torch::Tensor a,
  ...
  hgemm_t_8x8_sliced_k32_f16x8_pack_dbuf_async_kernel<BM, BN, BK, TM, TN, 8>
      <<<grid, block>>>(reinterpret_cast<half *>(a.data_ptr()),
                        reinterpret_cast<half *>(b.data_ptr()),
                        reinterpret_cast<half *>(c.data_ptr()), M, N, K);
}
...
void hgemm_t_16x8_sliced_k32_f16x8_pack_dbuf(torch::Tensor a, torch::Tensor b,
  ...
  hgemm_t_16x8_sliced_k32_f16x8_pack_dbuf_kernel<BM, BN, BK, TM, TN, 8>
      <<<grid, block>>>(reinterpret_cast<half *>(a.data_ptr()),
                        reinterpret_cast<half *>(b.data_ptr()),
                        reinterpret_cast<half *>(c.data_ptr()), M, N, K);
}
...
void hgemm_t_16x8_sliced_k32_f16x8_pack_dbuf_async(torch::Tensor a,
  ...
  hgemm_t_16x8_sliced_k32_f16x8_pack_dbuf_async_kernel<BM, BN, BK, TM, TN, 8>
      <<<grid, block>>>(reinterpret_cast<half *>(a.data_ptr()),
                        reinterpret_cast<half *>(b.data_ptr()),
                        reinterpret_cast<half *>(c.data_ptr()), M, N, K);
}
