                  ...
                  int N, int K)
{
    ...
    const int s_b_stage_offset = MMA_N * MMA_K;
    ...
    */
    ...
    /*
    这里特别注意，在v4代码中，smem和gmem中b矩阵layout一致，都是shape (8, 128) stride (128, 1)
    但在swizzle的代码中，gmem中的layout为shape (8, 128) stride (128, 1)，而在smem中的layout为shape(8, (16, 8)) stride(16, (1, 128)) （有层次layout）
    即gmem中数据layou是[8, 128], 而smem中layout为[8, 8 ,16]（为了符合swizzle的使用条件）
    [load_gmem_b_n, load_gmem_b_k]对应了gmem的[8, 128]区域中的坐标
    而[b_stage_idx, load_smem_b_n, load_smem_b_k]对应了smem的[8, 8, 16]区域中的坐标
    */
    int local_tid = tid % 16;           // [0-15] 每16个线程一组
    int b_stage_idx = tid / 16;
    int load_smem_b_n = local_tid / 2;
    int load_smem_b_k = (local_tid % 2) * 8;
    int load_gmem_b_n = local_tid / 2;
    int load_gmem_b_k = b_stage_idx * MMA_K + (local_tid % 2) * 8;
    ...
    int load_gmem_b_addr = load_gmem_b_n * INNER_DIM + load_gmem_b_k;
    uint32_t load_smem_b_addr = smem_b_base_ptr + (b_stage_idx * s_b_stage_offset + load_smem_b_n * MMA_K + swizzle_permuted_A_j<16>(load_smem_b_n, load_smem_b_k)) * sizeof(half);
...
#pragma unroll
    for (int k = 0; k < (K_STAGE - 1); ++k) {     // 0, 1
        ...
        uint32_t load_smem_a_addr = (smem_a_base_ptr + 
            (k * s_a_stage_offset + load_smem_a_m * MMA_K + swizzle_permuted_A_j<16>(load_smem_a_m, load_smem_a_k)) * sizeof(half));
        // 一次cp async载入一个64*16的tile
        CP_ASYNC_CG(load_smem_a_addr, &A[load_gmem_a_addr], 16);
    ...
    float RB_NORM = 0.0f;
    // // ldmatrix载入b矩阵
    // int lane_smem_b_n = lane_id / 2;
    // int lane_smem_b_k = lane_id % 2 * 8;
    // uint32_t lane_smem_b_addr = smem_b_base_ptr + 
    //     (lane_smem_b_n * MMA_K + lane_smem_b_k) * sizeof(half);
...
#pragma unroll
    for(int k = (K_STAGE - 1); k < NUM_K_TILES; k++){
        ...
        uint32_t load_smem_a_addr = (smem_a_base_ptr + 
            (gmem_load_stage * s_a_stage_offset + load_smem_a_m * MMA_K + swizzle_permuted_A_j<16>(load_smem_a_m, load_smem_a_k)) * sizeof(half));
        CP_ASYNC_CG(load_smem_a_addr, &A[load_a_gmem_addr], 16);
        CP_ASYNC_COMMIT_GROUP(); 

        // ldmatrix载入b矩阵
        int lane_smem_b_n = lane_id % 8;
        int lane_smem_b_k = (lane_id / 8 * 8) % 16;
        uint32_t lane_smem_b_addr = smem_b_base_ptr + 
            (b_stage * s_b_stage_offset + lane_smem_b_n * MMA_K + swizzle_permuted_A_j<16>(lane_smem_b_n, lane_smem_b_k)) * sizeof(half);
        // int lane_smem_b_n = lane_id % 8;
        ...
        uint32_t lane_smem_a_addr = smem_a_base_ptr + 
            (compute_stage * s_a_stage_offset + lane_smem_a_m * MMA_K + swizzle_permuted_A_j<16>(lane_smem_a_m, lane_smem_a_k)) * sizeof(half);
        LDMATRIX_X4(RA[0], RA[1], RA[2], RA[3], lane_smem_a_addr);
        ...
        // a_norm计算
        {
            ...
            RA_NORM[0] = __fmaf_rn(val_f2.x, val_f2.x, RA_NORM[0]);
            RA_NORM[0] = __fmaf_rn(val_f2.y, val_f2.y, RA_NORM[0]);
            ...
            val_f2 = __half22float2(val_h2);
            RA_NORM[1] = __fmaf_rn(val_f2.x, val_f2.x, RA_NORM[1]);
            RA_NORM[1] = __fmaf_rn(val_f2.y, val_f2.y, RA_NORM[1]);
        ...
        // b_norm计算
        {
            ...
            float2 val_f2 = __half22float2(val_h2);
            RB_NORM = __fmaf_rn(val_f2.x, val_f2.x, RB_NORM);
            RB_NORM = __fmaf_rn(val_f2.y, val_f2.y, RB_NORM);
    ...
    for(int k = 0; k < (K_STAGE - 1); k++){
        ...
        // LDMATRIX_X4(RA[0], RA[1], RA[2], RA[3], lane_smem_a_addr);
        uint32_t lane_smem_a_addr = smem_a_base_ptr + 
            (remain_compute_stage * s_a_stage_offset + lane_smem_a_m * MMA_K + swizzle_permuted_A_j<16>(lane_smem_a_m, lane_smem_a_k)) * sizeof(half);
        LDMATRIX_X4(RA[0], RA[1], RA[2], RA[3], lane_smem_a_addr);
