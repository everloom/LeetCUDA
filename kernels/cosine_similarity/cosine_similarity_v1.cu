...
#pragma unroll
    for (int i = 0; i < WARP_TILE_M; ++i) {
        if (lane_id % 4 == 0) {
            RA_NORM[i][0] += __shfl_sync(0xffffffff, RA_NORM[i][0], lane_id + 1);
            RA_NORM[i][0] += __shfl_sync(0xffffffff, RA_NORM[i][0], lane_id + 2);
            RA_NORM[i][0] += __shfl_sync(0xffffffff, RA_NORM[i][0], lane_id + 3);
    
            RA_NORM[i][1] += __shfl_sync(0xffffffff, RA_NORM[i][1], 1);
            RA_NORM[i][1] += __shfl_sync(0xffffffff, RA_NORM[i][1], lane_id + 2);
            RA_NORM[i][1] += __shfl_sync(0xffffffff, RA_NORM[i][1], lane_id + 3);
        }
    }
...
#pragma unroll
    for (int j = 0; j < WARP_TILE_N; ++j) {
        if (lane_id % 4 == 0) {
            RB_NORM[j] += __shfl_sync(0xffffffff, RB_NORM[j], lane_id + 1);
            RB_NORM[j] += __shfl_sync(0xffffffff, RB_NORM[j], lane_id + 2);
            RB_NORM[j] += __shfl_sync(0xffffffff, RB_NORM[j], lane_id + 3);
        }
    }
    // printf("here 2\n");
  {
...
int main(int argc, char *argv[]) {
  ...
  for (int j = 0; j < test_num; j++) {
    ...
    for (int k = 0; k < outer_repeat; k++) {
      double this_sec = perf_gemm<half>(launch_gemm_kernel_tn<2, 2048>,
