...
void launch_cosine_kernel_tn(half *a, half *b, half *c, int M, int N, int K)
{
    ...
    */
    ...
    // 这个代码将B矩阵的L2 cache固定了，保证B矩阵再L2中不会被A矩阵的数据驱逐，都是ai写的
    // 但发现还是没有效果
    cosine_kernel_tn<MMA_M, MMA_N, MMA_K, INNER_DIM, MMA_TILE_M, A_PAD, B_PAD, K_STAGE>
