...
float perf_gemm(void (*gpu_hgemm)(T *, T *, T *, int, int, int), int M, int N,
  ...
  if (err != cudaSuccess) { printf("Malloc A failed: %s\n", cudaGetErrorString(err)); return 0; }
  err = cudaMalloc(&d_b, size_b);
  if (err != cudaSuccess) { printf("Malloc B failed: %s\n", cudaGetErrorString(err)); return 0; }
  err = cudaMalloc(&d_c, size_c);
  if (err != cudaSuccess) { printf("Malloc C failed: %s\n", cudaGetErrorString(err)); return 0; }
  ...
  err = cudaGetLastError();
  if (err != cudaSuccess) {
      printf("Kernel Execution Failed (Async): %s\n", cudaGetErrorString(err));
      return 0.0f;
  }
